module.exports = (function() { 
    ...
    public.render = function() {
        ...
        for(var request, action, i = 0; i < batch.length; i++) {
			...
			request.url 		= batch[i].url || '/category';
            ...
            switch(true) {
                case batch[i].url.toLowerCase().indexOf('/category/create') === 0:  	
					action 	 			= require('./create');
					request.method 		= 'POST';
					break;
                case batch[i].url.toLowerCase().indexOf('/category/remove/') === 0:
					action 	 			= require('./remove');
					request.method 		= 'DELETE';
					request.variables	= batch[i].url.replace('/category/remove/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/category/restore/') === 0:
					action 	 			= require('./restore');
					request.method 		= 'PUT';
					request.variables	= batch[i].url.replace('/category/restore/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/category/update/') === 0:
					action 	 			= require('./update');
					request.method 		= 'PUT';
					request.variables	= batch[i].url.replace('/category/update/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/category/detail/') === 0:
					action 	 			= require('./detail');
					request.method 		= 'GET';
					request.variables	= batch[i].url.replace('/category/detail/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/category/list') === 0:
					action 	 			= require('./list');
					request.method 		= 'GET';
					break;
				default:
				...
				return function(next) {
					if(!virtualAction) {
						...
						if(self.results.length == JSON.parse(self.request.message).length) {
							...
							//Been trigger to the category action
					...
					//listen for a category action
        			self.controller.once('category-action-response', function(request, response) {
