module.exports = require('edenjs').extend(function() {
	...
	this.response = function() {
		...
		this._controller
			...
			.once('{{name}}-update-error', this._error.bind(this.capture()))
			//when it is successfull
			.once('{{name}}-update-success', this._success.bind(this.capture()))
			//Now call to remove the sample
			.trigger('{{name}}-update', this._request.variables[0], settings);
	};
