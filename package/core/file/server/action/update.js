module.exports = (function() {
	...
	public.render = function() {
		...
		this.controller
			...
			.once('file-update-error', function(error) {
				//setup an error response
				...
				self.controller.unlisten('file-update-success');
				//trigger that a response has been made
				self.controller.trigger('file-action-response', self.request, self.response);
			})
			//when it is successfull
			.once('file-update-success', function() {
				//set up a success response
				self.response.message = JSON.stringify({ error: false });
				//dont listen for error anymore
				self.controller.unlisten('file-update-error');
				//trigger that a response has been made
				self.controller.trigger('file-action-response', self.request, self.response);
			})
			//Now call to update the file
			.trigger('file-update', this.controller, this.request.variables[0], query);
	};
