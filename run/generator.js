module.exports = function(eve, local, args) {
    ...
    eve.trigger('generate-step-1', parameter.join('/'), eve);
    ...
    sequence.then(function(next) {
        for(var source, destination, i = 0; i < packages.length; i++) {
            ...
            source.packageName = vendor + '/' + packages[i];
            ...
            subsequence.then(function(source, destination, subnext) {
                eve.trigger('generate-step-2', source.packageName, eve);
            ...
            for (key in generatorFolders) {
                ...
                subsequence.then(function(folder, source, destination, subnext) {
                    eve.trigger('generate-step-3', source.packageName, folder, eve);
            ...
            //get output form
            subsequence.then(function(source, destination, subnext) {
                eve.trigger('generate-step-4', source.packageName, eve);
            ...
            subsequence.then(function(source, destination, output, subnext) {
                eve.trigger('generate-step-5', source.packageName, eve);
            ...
            //get validation
            subsequence.then(function(source, destination, subnext) {
                eve.trigger('generate-step-6', source.packageName, eve);
            ...
            subsequence.then(function(source, destination, subnext) {
                eve.trigger('generate-step-7', source.packageName, eve);
            ...
            subsequence.then(function(source, destination, files, subnext) {
                eve.trigger('generate-step-8', source.packageName, eve);
                for(var done = 0, i = 0; i < files.length; i++) {
                    files[i].getContent(function(file, error, data) {
                        ...
                        //Change variables
                        data = eden('string').replace(data, /{VENDOR}/g     , source.vendor);
            ...
            subsequence.then(function(source, subnext) {
                eve.trigger('generate-step-complete', source.packageName, eve);
    ...
    function convertObjectToString(object, level, isArray) {
        ...
        for (key in object) {
            ...
            else if (eden('array').isArray(object[key])) {
                output += JSON.stringify(object[key]);
            }
