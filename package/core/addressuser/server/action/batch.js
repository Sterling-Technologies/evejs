module.exports = (function() { 
    ...
    public.render = function() {
        if(!this.request.message.length) {
			...
			this.controller.trigger('addressuser-action-response', this.request, this.response);
        ...
        for(var request, action, i = 0; i < batch.length; i++) {
			...
			request.url 		= batch[i].url || '/addressuser';
            ...
            switch(true) {
                case batch[i].url.toLowerCase().indexOf('/addressuser/create') === 0:  	
					action 	 			= require('./create');
					request.method 		= 'POST';
					break;
                case batch[i].url.toLowerCase().indexOf('/addressuser/remove/') === 0:
					action 	 			= require('./remove');
					request.method 		= 'DELETE';
					request.variables	= batch[i].url.replace('/addressuser/remove/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/addressuser/restore/') === 0:
					action 	 			= require('./restore');
					request.method 		= 'PUT';
					request.variables	= batch[i].url.replace('/addressuser/restore/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/addressuser/update/') === 0:
					action 	 			= require('./update');
					request.method 		= 'PUT';
					request.variables	= batch[i].url.replace('/addressuser/update/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/addressuser/detail/') === 0:
					action 	 			= require('./detail');
					request.method 		= 'GET';
					request.variables	= batch[i].url.replace('/addressuser/detail/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/addressuser/list') === 0:
					action 	 			= require('./list');
					request.method 		= 'GET';
					break;
				default:
				...
				return function(next) {
					if(!virtualAction) {
						...
						if(self.results.length == JSON.parse(self.request.message).length) {
							...
							//Been trigger to the addressuser action
					...
					//listen for a addressuser action
        			self.controller.once('addressuser-action-response', function(request, response) {
