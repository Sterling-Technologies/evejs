jQuery.eve.base.define(function() {
	this.String = jQuery.classified(function() {
		/* Require
		-------------------------------*/
		/* Constants
		-------------------------------*/
		this.HTML_SPECIALCHARS 	= 100;
		this.HTML_ENTITIES 		= 200;
		this.ENT_NOQUOTES 		= 300;
		this.ENT_COMPAT 		= 400;
		this.ENT_QUOTES 		= 500;
		 ...
		 * @return string
		 */
		this.camelize = function(data, prefix) {
		 ...
		 * @return string
		 */
		this.charAt = function(data, index) {
		 ...
		 * @return string
		 */
		this.charCodeAt = function(data, index) {
		...
		this.concat = function() {
			var args = Array.prototype.slice.apply(arguments), data = args.shift();
		 ...
		 * @return bool
		 */
		this.endsWith = function(data, value) {
		 ...
		 * @return 	string
		 */
		this.hashToQuery = function(data) {
			// Query string
			var query = '', separator = '&';
		 ...
		 * @return string
		 */
		this.htmlEntities = function(data, quote, encode) {
			var symbol, map = this.__getHtmlMap(this.HTML_ENTITIES, quote);
			
			if(!map) {
				return this;
			}
			
			if(quote && quote === this.ENT_QUOTES) {
				map["'"] = '&#039;';
			}
		 ...
		 * @return string
		 */
		this.htmlEntityDecode = function(data, quote) {
			var symbol, entity,
				map = this.__getHtmlMap(this.HTML_ENTITIES, quote);
		 ...
		 * @return number
		 */
		this.indexOf = function(data, string) {
		 ...
		 * @return number
		 */
		this.lastIndexOf = function(data, string) {
		 ...
		 * @return array
		 */
		this.match = function(data, regex) {
		 ...
		 * @return string
		 */
		this.pathToArray = function(data) {
			if(data.indexOf('?') != -1) {
		 ...
		 * @return object
		 */
		this.pathToQuery = function(data) {
			if(data.indexOf('?') != -1) {
		 ...
		 * @return object
		 */
		this.queryToHash = function(data) {
			var hash = {};
		 ...
		 * @return string
		 */
		this.replace = function(data, needle, value) {
		 ...
		 * @return bool
		 */
		this.startsWith = function(data, value) {
		 ...
		 * @return number
		 */
		this.search = function(data, string) {
		 ...
		 * @return array
		 */
		this.split = this.explode = function(data, delimeter) {
		 ...
		 * @return string
		 */
		this.stripSlashes = function(data) {
		 ...
		 * @return string
		 */
		this.stripTags = function(data, allowed) {
		 ...
		 * @return string
		 */
		this.summarize = function(data, words) {
		 ...
		 * @return string
		 */
		this.titlize = function(data, prefix) {
			data = this.replace(data, prefix || '-', ' ')
		 ...
		 * @return string
		 */
		this.toLowerCase = this.toLower = 
		this.strToLower = function(data) {
		 ...
		 * @return string
		 */
		this.toUpperCase = this.toUpper =  
		this.strToUpper = function(data) {
		 ...
		 * @return string
		 */
		this.uncamelize = function(data, prefix) {
			prefix = prefix || '-';
		...
		/* Private Methods
		-------------------------------*/
		this.__getHtmlMap = function(table, quote) {
			var decimal, symbol, map = {},
				entities = { 38: '&amp;', 60: '&lt;', 62: '&gt;' };
			
			//check the table
			if (table !== this.HTML_SPECIALCHARS && table !== this.HTML_ENTITIES) {
				return false;
			}
			
			if (table === this.HTML_ENTITIES) {
