module.exports = function() {
	...
	this.listen('init', function() {
		this.path('category', 			__dirname)
			.path('category/action', 	__dirname + '/action')
			.path('category/event', 	__dirname + '/event');
	...
	.listen('start', function() {
		//get event path
		var self = this, events = this.category().path('event');
	...
	.listen('server-request', function(control, request, response) {
		//if path does not starts with /category
		if(request.path != '/category' && request.path.indexOf('/category/') !== 0) {
			//do nothing
			return;
		}
		...
		//trim the prefix
		var root 		= this.category().path('action'),
			path 		= request.path.replace('/category', ''),
			buffer 		= path.split('/'),
			action 		= root + '/index',
			variables 	= [];
		...
		//listen for response
		control.once('category-action-response', function(request, response) {
