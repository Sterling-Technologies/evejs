module.exports = function() {
	...
	.listen('start', function() {
		//get event path
		var self = this, events = this.file().path('event');
	...
	.listen('server-request', function(control, request, response) {
		...
		//trim the prefix
		var root 		= this.file().path('action'),
		...
		//listen for response
		control.once('file-action-response', function(request, response) {
