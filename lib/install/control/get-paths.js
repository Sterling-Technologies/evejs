module.exports = function(eden, local, deploy, next) {
	...
	var setPath = function(settings, next, json) {
		...
		deploy.path = (local +'/deploy/control').replace(/\//g, require('path').sep);
		json.control.path = ('./deploy/control').replace(/\//g, require('path').sep);
