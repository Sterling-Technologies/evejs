module.exports = (function() { 
    ...
    public.render = function() {
        ...
        for(var request, action, i = 0; i < batch.length; i++) {
			...
			request.url 		= batch[i].url || '/address';
            ...
            switch(true) {
                case batch[i].url.toLowerCase().indexOf('/address/create') === 0:  	
					action 	 			= require('./create');
					request.method 		= 'POST';
					break;
                case batch[i].url.toLowerCase().indexOf('/address/remove/') === 0:
					action 	 			= require('./remove');
					request.method 		= 'DELETE';
					request.variables	= batch[i].url.replace('/address/remove/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/address/restore/') === 0:
					action 	 			= require('./restore');
					request.method 		= 'PUT';
					request.variables	= batch[i].url.replace('/address/restore/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/address/update/') === 0:
					action 	 			= require('./update');
					request.method 		= 'PUT';
					request.variables	= batch[i].url.replace('/address/update/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/address/detail/') === 0:
					action 	 			= require('./detail');
					request.method 		= 'GET';
					request.variables	= batch[i].url.replace('/address/detail/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/address/list') === 0:
					action 	 			= require('./list');
					request.method 		= 'GET';
					break;
				default:
				...
				return function(next) {
					if(!virtualAction) {
						...
						if(self.results.length == JSON.parse(self.request.message).length) {
							...
							//Been trigger to the address action
					...
					//listen for a address action
        			self.controller.once('address-action-response', function(request, response) {
