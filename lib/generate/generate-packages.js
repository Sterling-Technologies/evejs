module.exports = function(eden, eve, parameter, packages, settings, vendor, paths, copy, next) {
	//loop through packages
	eden('array').each(packages, function(i, package) {
		//require the source path
		var config = require(paths.schema + vendor.name + '/' + package);
	...
	var render = function(content, data) {
		...
		content = eden('string').replace(content, /{VENDOR}/g	, vendor.name);
