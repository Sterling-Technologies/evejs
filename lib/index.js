module.exports = (function() {
	...
	public.run = function(local, action, section) {
		...
		var triggerBefore = action + '-' + section + '-before';
		var isGenerate = action === 'generate';

		//generate has different file naming and trigger naming.
		if (isGenerate) {
			build = __dirname + '/../lib/generator.js';
			triggerBefore = 'generate-before';
		}
		
		this.trigger(triggerBefore, local, action, section);
		...
		//generate has different arguments and does not require config.
		if (isGenerate) {
			//require the file and let it do the rest
			require(build)(this, local, config, section);
