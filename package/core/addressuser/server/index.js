module.exports = function() {
	...
	//want to access this
	this.addressuser = function() {
		return require('./factory').load(this);
	};
	
	//on init set the paths
	this.listen('init', function() {
		this.path('addressuser', __dirname)
			.path('addressuser/action', __dirname + '/action')
			.path('addressuser/event', __dirname + '/event');
	...
	.listen('start', function() {
		//get event path
		var self = this, events = this.addressuser().path('event');
	...
	.listen('server-request', function(control, request, response) {
		//if path does not starts with /addressuser
		if(request.path != '/addressuser' && request.path.indexOf('/addressuser/') !== 0) {
			//do nothing
			return;
		}
		...
		//trim the prefix
		var root 		= this.addressuser().path('action'),
			path 		= request.path.replace('/addressuser', ''),
			buffer 		= path.split('/'),
			action 		= root + '/index',
			variables 	= [];
		...
		//listen for response
		control.once('addressuser-action-response', function(request, response) {
