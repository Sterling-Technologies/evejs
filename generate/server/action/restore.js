module.exports = require('edenjs').extend(function() {
	...
	this.response = function() {
		...
		this._controller
			...
			.once('{{name}}-restore-error', this._error.bind(this.capture()))
			//when it is successfull
			.once('{{name}}-restore-success', this._success.bind(this.capture()))
			//Now call to remove the sample
			.trigger('{{name}}-restore', this._request.variables[0]);
	};
