module.exports = function() {
	...
	this.Folder(events).getFiles(null, false, function(error, files) {
		...
		//is it installed?
		this.package('user').search().setRange(1).getRow(function(error) {
			//if no table
			if(error && error.code === 'ER_NO_SUCH_TABLE') {
				console.log('\x1b[33m%s\x1b[0m', 'Installing user package ...');
				require('./install')(this.database().setup, function(error) {
					...
					console.log('\x1b[32m%s\x1b[0m', 'user package installed!');
				});
			} 
		}.bind(this));
	}.bind(this));
