	...
	settings.getContent(function(error, content) {
		...
		.on('all', function(event, path, stats) {
			...
			var destination,
				...
				pathArray = path.substr(local.length).split(sep);
			...
			&& pathArray[4] == 'control') {
				destination = config.control.path 
					+ sep + 'application' + sep + 'package' + sep 
					+ pathArray[2] + sep 
					+ pathArray[3] + sep 
					+ pathArray.slice(5).join(sep);
			...
			&& pathArray[2] == 'control') {
				destination = config.control.path
					+ sep + 'application' + sep + 'config' + sep
					+ pathArray.slice(3).join(sep);
			}
