module.exports = function() {
	...
	/* Private Properties
	-------------------------------*/
	var _user = { type : String, required : true };
	...
	public.__construct = function(controller) {
		...
		var store  = controller.address().store();

		// copy the schema
		this.schema = store.schema;

		// inject required fields
		this.schema.user = _user;

		//define schema
		this.definition = new schema(this.schema, { collection : 'addressuser' });
		
		this.store = mongoose.model('addressuser', this.definition);
	};
	...
	public.getDetail = function(id, callback, lean) {
		var query = this.findOne({ _post: id });
