module.exports = function() {
	...
	this.listen('init', function() {
		this.path('categorypost', __dirname)
			.path('categorypost/action', __dirname + '/action')
			.path('categorypost/event', __dirname + '/event');
	...
	.listen('start', function() {
		//get event path
		var self = this, events = this.categorypost().path('event');
	...
	.listen('server-request', function(control, request, response) {
		...
		//trim the prefix
		var root 		= this.categorypost().path('action'),
