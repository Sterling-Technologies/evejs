module.exports = (function() { 
    ...
    public.render = function() {
        ...
        for(var request, action, i = 0; i < batch.length; i++) {
			...
			request.url 		= batch[i].url || '/postuser';
            ...
            switch(true) {
                case batch[i].url.toLowerCase().indexOf('/postuser/create') === 0:  	
					action 	 			= require('./create');
					request.method 		= 'POST';
					break;
                case batch[i].url.toLowerCase().indexOf('/postuser/remove/') === 0:
					action 	 			= require('./remove');
					request.method 		= 'DELETE';
					request.variables	= batch[i].url.replace('/postuser/remove/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/postuser/restore/') === 0:
					action 	 			= require('./restore');
					request.method 		= 'PUT';
					request.variables	= batch[i].url.replace('/postuser/restore/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/postuser/update/') === 0:
					action 	 			= require('./update');
					request.method 		= 'PUT';
					request.variables	= batch[i].url.replace('/postuser/update/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/postuser/detail/') === 0:
					action 	 			= require('./detail');
					request.method 		= 'GET';
					request.variables	= batch[i].url.replace('/postuser/detail/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/postuser/list') === 0:
					action 	 			= require('./list');
					request.method 		= 'GET';
					break;
				default:
				...
				return function(next) {
					if(!virtualAction) {
						...
						if(self.results.length == JSON.parse(self.request.message).length) {
							...
							//Been trigger to the postuser action
					...
					//listen for a postuser action
        			self.controller.once('postuser-action-response', function(request, response) {
