module.exports = function() {
	...
	.listen('start', function() {
		//get event path
		var self = this, events = this.address().path('event');
	...
	.listen('server-request', function(control, request, response) {
		//if path does not starts with /address
		if(request.path != '/address' && request.path.indexOf('/address/') !== 0) {
			//do nothing
			return;
		}
		...
		//trim the prefix
		var root 		= this.address().path('action'),
		...
		//listen for response
		control.once('address-action-response', function(request, response) {
