module.exports = (function() {
	...
	public.render = function() {
		...
		this.controller
			...
			.once('category-update-error', function(error) {
				//setup an error response
				...
				self.controller.unlisten('category-update-success');
				//trigger that a response has been made
				self.controller.trigger('category-action-response', self.request, self.response);
			})
			//when it is successfull
			.once('category-update-success', function() {
				//set up a success response
				self.response.message = JSON.stringify({ error: false });
				//dont listen for error anymore
				self.controller.unlisten('category-update-error');
				//trigger that a response has been made
				self.controller.trigger('category-action-response', self.request, self.response);
			})
			//Now call to update the category
