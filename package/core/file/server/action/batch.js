        ...
        for(var request, action, i = 0; i < batch.length; i++) {
            ...
            switch(true) {
                ...
                case batch[i].url.toLowerCase().indexOf('/file/remove/') === 0:
					...
					request.variables	= batch[i].url.replace('/file/remove/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/file/restore/') === 0:
					action 	 			= require('./restore');
					request.method 		= 'PUT';
					request.variables	= batch[i].url.replace('/file/restore/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/file/update/') === 0:
					action 	 			= require('./update');
					request.method 		= 'PUT';
					request.variables	= batch[i].url.replace('/file/update/', '').split('/');
					break;
                case batch[i].url.toLowerCase().indexOf('/file/detail/') === 0:
					action 	 			= require('./detail');
					request.method 		= 'GET';
					request.variables	= batch[i].url.replace('/file/detail/', '').split('/');
					break;
				...
				return function(next) {
					if(!virtualAction) {
						...
						if(self.results.length == JSON.parse(self.request.message).length) {
							...
							//Been trigger to the file action
					...
					//listen for a file action
        			self.controller.once('file-action-response', function(request, response) {
